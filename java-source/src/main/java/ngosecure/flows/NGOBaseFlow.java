...
/**
 * An abstract FlowLogic class that is subclassed by the ngo transaction flows to
 * provide helper methods and classes.
 */
abstract class NGOBaseFlow extends FlowLogic<SignedTransaction> {
    ...
    StateAndRef<NGOTransaction> getNGOTransactionByLinearId(UniqueIdentifier linearId) throws FlowException {
        ...
        List<StateAndRef<NGOTransaction>> ngoTxns = getServiceHub().getVaultService().queryBy(NGOTransaction.class, queryCriteria).getStates();
        if (ngoTxns.size() != 1) {
            throw new FlowException(String.format("NGOTransaction with id %s not found.", linearId));
        }
        return ngoTxns.get(0);
    }
